...
#
# Script generated by Vivado on Mon Jun 17 14:35:54 +0200 2024
...
#
# Usage        : x_RAM.sh
#                x_RAM.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                x_RAM.sh [-reset_run]
#                x_RAM.sh [-reset_log]
#                x_RAM.sh [-help]
#
...
#                script (type 'x_RAM.sh -help' for more information).
#
...
# script info
echo -e "x_RAM.sh - Script generated by export_simulation (Vivado v2024.1 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./x_RAM.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  vcs $vcs_elab_opts xil_defaultlib.x_RAM xil_defaultlib.glbl -o x_RAM_simv
}

# RUN_STEP: <simulate>
simulate()
{
  ./x_RAM_simv $vcs_sim_opts -do simulate.do
}
...
reset_run()
{
  files_to_remove=(ucli.key x_RAM_simv vlogan.log vhdlan.log compile.log elaborate.log simulate.log .tmp_log .vlogansetup.env .vlogansetup.args .vcs_lib_lock scirocco_command.log lib_sc.so 64 AN.DB csrc x_RAM_simv.daidir vcs_lib c.obj)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./x_RAM.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: x_RAM.sh [-help]\n\
Usage: x_RAM.sh [-step]\n\
Usage: x_RAM.sh [-lib_map_path]\n\
Usage: x_RAM.sh [-reset_run]\n\
Usage: x_RAM.sh [-reset_log]\n\
Usage: x_RAM.sh [-keep_index]\n\
Usage: x_RAM.sh [-noclean_files]\n\n\
